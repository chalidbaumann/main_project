...
function setup() {
    ...
    //polarity
    /*points = font.textToPoints('hi', 50, 50, 30, {
        sampleFactor: 5,
        simplifyThreshold: 0
    })
    bounds = font.textBounds('hi', 50, 50, 30);*/ //bounding box für Text ?
    //confidence
    unsure = 20;
    sure = 100;
    frameRate(0.4);
...
function draw() {
    ...
    for (let i = 0; i < points.length; i++) {
        ...
        vertex( //Angaben zur Form innerhalb Funktion beginShape() und endShape()
            p.x * width / bounds.w + sin(20 * p.y / bounds.h + millis() / 1000) * width / 30,
            p.y * height / bounds.h
        );
    }
    endShape(CLOSE);*/
